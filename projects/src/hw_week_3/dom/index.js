 ...
 */
function createDivWithText(loftschool) {
...
function deleteTextNodes(where) {
  for (let i = 0; i < where.childNodes.length; i++) {
    const nodes = where.childNodes[i];
    if (nodes.nodeType === Element.TEXT_NODE) {
...
function deleteTextNodesRecursive(where) {
  for (let i = 0; i < where.childNodes.length; i++) {
    ...
    if (nodes.nodeType === Element.TEXT_NODE) {
      where.removeChild(nodes);
      i--;
    } else if (nodes.nodeType === Element.ELEMENT_NODE) {
      deleteTextNodesRecursive(nodes);
    }
  }
}
...
// тут я немного не разобрался,надо будет разбор посмотреть
/*
 Задание 8 *:
